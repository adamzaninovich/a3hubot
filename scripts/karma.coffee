...
class Karma
  ...
  constructor: (@robot) ->
    ...
    @increment_responses = [
      "+1!", "gained a level!", "is on the rise!", "leveled up!", "knows how to play the crowd!"
    ]

    @decrement_responses = [
      "took a hit! Ouch.", "took a dive.", "lost a life.", "lost a level.", "is pretty much screwed."
    ]

    @robot.brain.on 'loaded', =>
      if @robot.brain.data.karma
        @cache = @robot.brain.data.karma
      @cache['michaelhatesyouall'] = -100
  ...
  selfDeniedResponses: (name) ->
    @self_denied_responses = [
      ...
      "#{name}, I have 4 words for you... GTFO!"
    ]
